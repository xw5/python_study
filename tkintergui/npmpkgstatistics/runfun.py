    ...
    try:
        with open('{}/node_modules/{}/package.json'.format(proPath, dd), "r", encoding='utf-8') as f:
            ...
            for i, item in enumerate(keys):
                sht[index, i + 1].value = row_data_pkjson.get(item, '未知')
    except:
        sht[index, 1].value = '访问出错'
def statisticsAction(proPath, outPath, keys, progressBack, sucessBack, uiName):
    # 读取文件数据
    with open(f'{proPath}/package.json', "r", encoding='utf-8') as f:
        ...
        # 读取每一条json数据
        for d in row_data:
            if d == 'devDependencies' or d == 'dependencies':
